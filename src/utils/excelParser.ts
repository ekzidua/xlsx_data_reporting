...
const extractCategory = (description: string): string => {
  ...
  if (lowerDesc.includes('такса') && (lowerDesc.includes('обслужван') || lowerDesc.includes('обслужване'))) return 'ТАКСА ОБСЛУЖВАНЕ';
  if (lowerDesc.includes('service fee')) return 'ТАКСА ОБСЛУЖВАНЕ';
